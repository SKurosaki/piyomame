
$(function(addSearchWord){
  // -- 1 : MA --
  $('.abomination').append('Abomination Emil Blonsky アボミネーション エミルブロンスキー あぼみねーしょん えみるぶろんすきー').next().children('.fan-art_img').addClass('tab1-i tab1_1-i');
  $('.antman').append('Ant Man Scott Lang アントマン スコットラング あんとまん すこっとらんぐ').next().children('.fan-art_img').addClass('tab1-i tab1_2-i tab1_2_11-i');
  $('.bucky').append('James Buchanan Barnes Bucky Winter Soldier バッキーバーンズ ウィンターソルジャー ウィンソル ばっきーばーんず うぃんたーそるじゃー うぃんそる 冬兵士').next().children('.fan-art_img').addClass('tab1-i tab1_1-i tab1_2-i');
  $('.cancer').append('Cancer キャンサー きゃんさー').next().children('.fan-art_img').addClass('tab1-i tab1_1-i');
  $('.cap').append('Captain America Steve Rogers キャプテンアメリカ スティーブロジャース きゃぷてんあめりか すてぃーぶろじゃーす キャップ').next().children('.fan-art_img').addClass('tab1-i tab1_2-i tab1_2_3-i');
  $('.carina').append('Carina カリーナ かりーな').next().children('.fan-art_img').addClass('tab1-i tab1_3-i');
  $('.collector').append('Collector Taneleer Tivan コレクター タニリーアティバン これくたー たにりーあてぃばん').next().children('.fan-art_img').addClass('tab1-i tab1_1-i');
  $('.dark_elf').append('Dark Elf ダークエルフ だーくえるふ').next().children('.fan-art_img').addClass('tab1-i tab1_1-i');
  $('.deadpool').append('Deadpool デッドプール デップー でっどぷーる でっぷー 俺ちゃん').next().children('.fan-art_img').addClass('tab1-i tab1_2-i');
  $('.destroyer').append('Destroyer デストロイヤー ですとろいやー').next().children('.fan-art_img').addClass('tab1-i tab1_1-i');
  $('.doom').append('Doctor Dr. Doom Victor von Doom ヴィクターフォンドゥーム ドクタードゥーム ゔぃくたーふぉんどぅーむ どくたーどぅーむ').next().children('.fan-art_img').addClass('tab1-i tab1_1-i');
  $('.drax').append('Drax the Destroyer ドラックスザデストロイヤー どらっくすざですとろいやー').next().children('.fan-art_img').addClass('tab1-i tab1_2-i tab1_2_17-i');
  $('.frigga').append('Frigga フリッガ ふりっが').next().children('.fan-art_img').addClass('tab1-i tab1_3-i');
  $('.fury').append('Nick Fury ニックフューリー にっくふゅーりー 長官').next().children('.fan-art_img').addClass('tab1-i tab1_3-i tab1_3_1-i');
  $('.gamora').append('Gamora ガモーラ がもーら').next().children('.fan-art_img').addClass('tab1-i tab1_2-i tab1_2_14-i');
  $('.graviton').append('Graviton Franklin Hall フランクリンホール グラビトン ぐらびとん ふらんくりんほーる').next().children('.fan-art_img').addClass('tab1-i tab1_1-i');
  $('.green_goblin').append('Green Goblin グリーンゴブリン ぐりーんごぶりん').next().children('.fan-art_img').addClass('tab1-i tab1_1-i');
  $('.groot').append('Groot グルート ぐるーと').next().children('.fan-art_img').addClass('tab1-i tab1_2-i tab1_2_15-i');
  $('.hawk').append('Hawkeye Clint Barton ホークアイ クリントバートン ほーくあい くりんとばーとん 鷹さん').next().children('.fan-art_img').addClass('tab1-i tab1_2-i tab1_2_6-i');
  $('.heimdall').append('Heimdall ヘイムダル へいむだる').next().children('.fan-art_img').addClass('tab1-i tab1_3-i');
  $('.hit_mon').append('Hit Monkey ヒットモンキー ひっともんきー').next().children('.fan-art_img').addClass('tab1-i tab1_1-i');
  $('.hogun').append('Hogun ホーガン ほーがん').next().children('.fan-art_img').addClass('tab1-i tab1_3-i');
  $('.hulk').append('Hulk Bruce Banner ハルク ブルースバナー はるく ぶるーすばなー 博士').next().children('.fan-art_img').addClass('tab1-i tab1_2-i tab1_2_4-i');
  $('.ironfist').append('Iron Fist Daniel Danny Rand アイアンフィスト ダニーランド').next().children('.fan-art_img').addClass('tab1-i tab1_2-i');
  $('.jane').append('Jane Foster ジェーンフォスター じぇーんふぉすたー').next().children('.fan-art_img').addClass('tab1-i tab1_3-i');
  $('.juggernaut').append('Juggernaut ジャガーノート じゃがーのーと').next().children('.fan-art_img').addClass('tab1-i tab1_1-i');
  $('.kurse').append('Algrim the Strong Kurse アルグリムザストロング カース あるぐりむざすとろんぐ かーす').next().children('.fan-art_img').addClass('tab1-i tab1_1-i');
  $('.loki').append('Loki Laufeyson ロキ ろき 邪神 ろきちゃん ロキちゃん 로키').next().children('.fan-art_img').addClass('tab1-i tab1_1-i tab1_1_1-i');
  $('.magneto').append('Magneto Erik Magnus Lehnsherr マグニート エリック レーンシャー まぐにーと えりっく れーんしゃー').next().children('.fan-art_img').addClass('tab1-i tab1_1-i tab1_1_6-i');
  $('.malekith').append('Malekith the Accursed マレキス まれきす').next().children('.fan-art_img').addClass('tab1-i tab1_1-i tab1_1_5-i');
  $('.mystique').append('Mystique Raven Darkhölme ミスティーク レイヴンダークホルム みすてぃーく れいゔんだーくほるむ').next().children('.fan-art_img').addClass('tab1-i tab1_1-i');
  $('.modok').append('MODOK M.O.D.O.K. モードック もーどっく').next().children('.fan-art_img').addClass('tab1-i tab1_1-i');
  $('.nat').append('Black Widow Natasha Romanoff ブラックウィドウ ナターシャロマノフ ぶらっくうぃどう なたーしゃろまのふ ロマ姐 姐さん').next().children('.fan-art_img').addClass('tab1-i tab1_2-i tab1_2_5-i');
  $('.nebula').append('Nebula ネビュラ ねびゅら').next().children('.fan-art_img').addClass('tab1-i tab1_1-i tab1_1_8-i');
  $('.odin').append('Odin オーディン おーでぃん おでんとーちゃん 父上').next().children('.fan-art_img').addClass('tab1-i tab1_3-i tab1_3_5-i');
  $('.pepper').append('Pepper Potts ペッパーポッツ ペッパーポッツ').next().children('.fan-art_img').addClass('tab1-i tab1_3-i tab1_3_4-i');
  $('.quick-m').append('Quicksilver Pietro Maximoff クイックシルバー ピエトロマキシモフ クイシル くいっくしるばー ぴえとろまきしもふ くいしる').next().children('.fan-art_img').addClass('tab1-i tab1_2-i tab1_2_9-i');
  $('.quick-x').append('Quicksilver Peter Maximoff クイックシルバー ピーターマキシモフ クイシル くいっくしるばー ぴーたーまきしもふ くいしる').next().children('.fan-art_img').addClass('tab1-i tab1_2-i tab1_2_9-i');
  $('.red_skull').append('Red Skull Johann Schmidt レッドスカル ヨハンシュミット レッスカさん れっどすかる よはんしゅみっと れっすかさん 레드 스컬').next().children('.fan-art_img').addClass('tab1-i tab1_1-i tab1_1_3-i');
  $('.rocket').append('Rocket Raccoon ロケットラクーン ろけっとらくーん').next().children('.fan-art_img').addClass('tab1-i tab1_2-i tab1_2_16-i');
  $('.ronan').append('Ronan the Accuser ロナンジアキューザー ロナンザアキューザー マジェスティ－ ろなんじあきゅーざー ろなんざあきゅーざー 로난 ').next().children('.fan-art_img').addClass('tab1-i tab1_1-i tab1_1_2-i');
  $('.sam').append('Falcon Sam Wilson Samuel Wilson ファルコン サムウィルソン サミュエルウィルソン ふぁるこん さむうぃるそん さみゅえるうぃるそん').next().children('.fan-art_img').addClass('tab1-i tab1_2-i tab1_2_10-i');
  $('.shocker').append('Herman Schultz Shocker ハーマンシュルツ ショッカー はーまんしゅるつ しょっかー').next().children('.fan-art_img').addClass('tab1-i tab1_1-i');
  $('.spidey').append('Spider-Man Spider Man Peter Benjamin Parker Spidey スパイダーマン ピーターパーカー スパイディ すぱいだーまん ぴーたーぱーかー すぱいでぃ 親愛なる隣人').next().children('.fan-art_img').addClass('tab1-i tab1_2-i tab1_2_12-i');
  $('.star_lord').append('Star-Lord Star Lord Peter Quill スターロード ピータークイル スタロ すたーろーど ぴーたーくいる すたろ').next().children('.fan-art_img').addClass('tab1-i tab1_2-i tab1_2_13-i');
  $('.thor').append('Thor Odinson ソー そー 兄上 토르').next().children('.fan-art_img').addClass('tab1-i tab1_2-i tab1_2_1-i');
  $('.tony').append('Iron Man IronMan  Tony Stark アイアンマン トニースターク あいあんまん とにーすたーく 社長').next().children('.fan-art_img').addClass('tab1-i tab1_2-i tab1_2_2-i');
  $('.ultron').append('Ultron ウルトロン うるとろん').next().children('.fan-art_img').addClass('tab1-i tab1_1-i tab1_1_4-i');
  $('.venom').append('Venom ヴェノム ゔぇのむ べのむ').next().children('.fan-art_img').addClass('tab1-i tab1_1-i tab1_1_7-i');
  $('.wasp').append('Wasp ワスプ わすぷ').next().children('.fan-art_img').addClass('tab1-i tab1_2-i tab1_2_7-i');
  $('.yondu').append('Yondu Udonta ヨンドゥウドンタ よんどぅうどんた').next().children('.fan-art_img').addClass('tab1-i tab1_2-i');
  $('.zemo').append('Baron Zemo バロンジモ ばろんじも').next().children('.fan-art_img').addClass('tab1-i tab1_1-i');
  $('.zola').append('Arnim Zola アーニムゾラ あーにむぞら').next().children('.fan-art_img').addClass('tab1-i tab1_1-i');


  //DWA
  $('.akira').append('Akira Akatsuki アカツキアキラ あかつきあきら').next().children('.fan-art_img').addClass('tab1-i tab1_2-i');
  $('.chris_t').append('Chris Taylor クリステイラー くりすていらー').next().children('.fan-art_img').addClass('tab1-i tab1_2-i');
  $('.edward').append('Edward Grant エドワードグラント えどわーどぐらんと').next().children('.fan-art_img').addClass('tab1-i tab1_2-i');
  $('.hikaru').append('Hikaru Akatsuki アカツキヒカル あかつきひかる').next().children('.fan-art_img').addClass('tab1-i tab1_2-i');
  $('.jessica').append('Jessica Shannon ジェシカシャノン じぇしかしゃのん').next().children('.fan-art_img').addClass('tab1-i tab1_2-i');
  $('.nozomu').append('Doctior Dr. Nozomu Akatsuki アカツキノゾム あかつきのぞむ あかつき博士 アカツキ博士').next().children('.fan-art_img').addClass('tab1-i tab1_2-i');

// ).next().children('.fan-art_img').addClass('tab1-i tab1_1-i');GOTG = 가디언즈 오브 갤럭시 or 가오갤


  //-- 2 : DI --
  //BH6
  $('.baymax').append('Baymax ベイマックス べいまっくす');
  $('.fred').append('Fred フレッド フレディ ふれっど');
  $('.gogo').append('Gogo Tomago ゴーゴートマゴ ゴーゴータマゴ ごーごーとまご ごーごーたまご ごご');
  $('.hiro').append('Hiro Hamada ヒロハマダ ひろはまだ');
  $('.honey').append('Honey Lemon ハニーレモン はにーれもん はにれも');
  $('.tadashi').append('Tadashi Hamada タダシハマダ ただしはまだ');
  $('.wasabi').append('Wasabi ワサビ わさび');
  $('.yokai').append('Yokai ヨーカイ カブキマスクマン よーかい かぶきますくまん');
  //LK
  $('.banzai').append('Banzai バンザイ ばんざい');
  $('.ed').append('Ed エド えど').next().children('.fan-art_img').addClass('tab2-i tab2_1-i tab2_1_3-i');
  $('.hyenas').append('Hyenas ハイエナトリオ はいえなとりお');
  $('.mufasa').append('Mufasa ムファサ むふぁさ').next().children('.fan-art_img').addClass('tab2-i tab2_1-i tab2_1_2-i');
  $('.scar').append('Scar スカー すかー おじさん すかーおじさん').next().children('.fan-art_img').addClass('tab2-i tab2_1-i tab2_1_1-i');
  $('.shenzi').append('Shenzi シェンジ しぇんじ');
  $('.zazu').append('Zazu ザズー ざずー').next().children('.fan-art_img').addClass('tab2-i tab2_1-i tab2_1_6-i');


  //NBC
  $('.barrel').append('Barrel バレル ばれる').next().children('.fan-art_img').addClass('tab2-i tab2_2-i tab2_2_4-i');
  $('.boogie').append('Oogie Boogie ウーギーブギー うーぎーぶぎー 親分').next().children('.fan-art_img').addClass('tab2-i tab2_2-i tab2_2_1-i');
  $('.lock').append('Lock ロック ろっく').next().children('.fan-art_img').addClass('tab2-i tab2_2-i tab2_2_2-i');
  $('.lsb').append('子鬼トリオ');
  $('.jack').append('Jack Skellington ジャックスケリントン パンプキンキング じゃっくすけりんとん ぱんぷきんきんぐ');
  $('.sally').append('Sally サリー さりー');
  $('.shock').append('Shock ショック しょっく').next().children('.fan-art_img').addClass('tab2-i tab2_2-i tab2_2_3-i');
  $('.zero').append('Zero ゼロ ぜろ');
  //MAL
  $('.diaval').append('Diaval ディアヴァル でぃあゔぁる').next().children('.fan-art_img').addClass('tab2-i');
  $('.maleficent').append('Maleficent マレフィセント まれふぃせんと').next().children('.fan-art_img').addClass('tab2-i');

  //POC
  $('.jack_pc').append('Jack the Monkey ジャックザモンキー じゃっくざもんきー さる 猿').next().children('.fan-art_img').addClass('tab2-i');


  //PAN
  $('.hook').append('Captain James Hook ジェームズフック フック船長 じぇーむずふっく').next().children('.fan-art_img').addClass('tab2-i');

  //-- 3 : SW --
  $('.boba').append('Boba Fett ボバフェット ぼばふぇっと').next().children('.fan-art_img').addClass('tab3-i tab3_3-i tab3_3_2-i');
  $('.ewok').append('Ewok イウォーク いうぉーく').next().children('.fan-art_img').addClass('tab3-i tab3_3-i tab3_3_1-i');
  $('.jango').append('Jango Fett ジャンゴフェット じゃんごふぇっと').next().children('.fan-art_img').addClass('tab3-i tab3_3-i tab3_3_3-i');
  $('.leia').append('Leia Organa Princess Leia レイアオーガナ レイア姫 れいあおーがな れいあひめ 将軍').next().children('.fan-art_img').addClass('tab3-i tab3_2-i tab3_2_1-i');
  $('.maul').append('Darth Maul ダースモール だーすもーる').next().children('.fan-art_img').addClass('tab3-i tab3_1-i tab3_1_2-i');
  $('.ren').append('Kylo Ren カイロレン かいろれん').next().children('.fan-art_img').addClass('tab3-i tab3_1-i tab3_1_3-i');
  $('.r2d2').append('R2-D2 R2D2 アールツーディーツー あーるつーでぃーつー').next().children('.fan-art_img').addClass('tab3-i tab3_2-i tab3_2_4-i');
  $('.stormtrooper').append('Stormtrooper ストームトルーパー すとーむとるーぱー').next().children('.fan-art_img').addClass('tab3-i tab3_1-i tab3_1_2-i');
  $('.vader').append('Darth Vader ダースベイダー だーすべいだー 米田').next().children('.fan-art_img').addClass('tab3-i tab3_1-i tab3_1_1-i');

  //-- 4 : FILM --
    //-- 4_1 : Pacific Rim
    $('.hermann').append('hermann gottlieb ハーマンゴットリーブ はーまんごっどりーぶ').next().children('.fan-art_img').addClass('tab4-i tab4_1-i tab4_1_3-i ');

    //-- 4_2 : Lord of the Ring / Hobbit
    $('.grima').append('Grima Gríma Wormtongue グリマ ぐりま 蛇の舌').next().children('.fan-art_img').addClass('tab4-i tab4_2-i tab4_2_1-i');
    $('.legolas').append('Legolas レゴラス れごらす').next().children('.fan-art_img').addClass('tab4-i tab4_2-i tab4_2_2-i');
    $('.treebeard').append('Treebeard Ent Onodrim エント オノドリム ファンゴルン えんと おのどりむ ふぁんごるん きのひげ 木の髭').next().children('.fan-art_img').addClass('tab4-i tab4_2-i');

    //-- 4_3 : Hobbit
    $('.legolas').append('Legolas レゴラス れごらす').next().children('.fan-art_img').addClass('tab4-i tab4_3-i tab4_3_2-i');
    $('.thranduil').append('Thranduil スランドゥイル すらんどぅいる スラ様 すら様 ぱぱ').next().children('.fan-art_img').addClass('tab4-i tab4_3-i tab4_3_1-i');


    //-- 4_4 : Star Trek
    $('.khan').append('John Harrison Khan ジョンハリソン カーン じょんはりそん かーん').next().children('.fan-art_img').addClass('tab4-i tab4_4-i tab4_4_1-i');
    $('.scotty').append('Montgomery Scott Scotty モンゴメリースコット スコッティ もんごめりーすこっと すこってぃ').next().children('.fan-art_img').addClass('tab4-i tab4_4-i tab4_4_2-i');


  //-- 5 : TV DRAMA --
  // SH
  $('.sherlock').append('Sherlock Holmes シャーロックホームズ しゃーろっくほーむず').next().children('.fan-art_img').addClass('tab5-i tab5_1-i tab5_1_1-i');
  $('.john').append('John Watson ジョンワトソン じょんわとそん').next().children('.fan-art_img').addClass('tab5-i tab5_1-i tab5_1_2-i');


  //-- 8 : THEATER --
    //-- 8_1 : Coriolanus
    $('.coriolanus').append('Coriolanus コリオレイナス こりおれいなす').next().children('.fan-art_img').addClass('tab8-i tab8_1-i tab8_1_1-i');

    //-- 8_2 : Frankenstein
    $('.creature').append('Creature クリーチャー くりーちゃー 怪物').next().children('.fan-art_img').addClass('tab8-i tab8_2-i tab8_2_2-i');
    $('.f_creature').append('Female Creature 女性クリーチャー じょせいりーちゃー').next().children('.fan-art_img').addClass('tab8-i tab8_2-i tab8_2_2-i');
    $('.frankenstein').append('Frankenstein フランケンシュタイン博士 ふらんけんしゅたいん').next().children('.fan-art_img').addClass('tab8-i tab8_2-i tab8_2_1-i');




  // $('.').append('');
  // $('.').append('');
  // $('.').append('');
  // $('.').append('');
  // $('.').append('');
  // $('.').append('');
  // $('.').append('');
  // $('.').append('');
  // $('.').append('');
  // $('.').append('');
  // $('.').append('');
  // $('.').append('');
  // $('.').append('');
  // $('.').append('');
  // $('.').append('');
  // $('.').append('');
  // $('.').append('');
  // $('.').append('');
  // $('.').append('');
  // $('.').append('');

  //-- SP --
  $('.bs').append('後姿');
  $('.dwa').append('DWA');
  $('.gb').append('神兄弟').next().children('.fan-art_img').addClass('tab6-i tab6_1-i');
  $('.gn').append('銀魂');
  $('.ic').append('アイコン');
  $('.ntl').append('National Theatre Live ナショナル・シアター・ライヴ ナショナルシアターライブ なしょなるしあたーらいぶ');
  $('.pt').append('ペットボトル');
  $('.sga').append('信号トリオA 信号トリオB').next().children('.fan-art_img').addClass('tab6-i tab6_3-i');
  $('.tbt').append('ティムバートン風');
  $('.yb').append('弟組').next().children('.fan-art_img').addClass('tab6-i tab6_2-i');
  $('.usa').append('うさぎ');


  //++ ACTOR ++
  $('.aaron_t_j').append('Aaron Taylor Johnson アーロンテイラージョンソン あーろんていらーじょんそん');
  $('.akinnuoye_a').append('Adewale Akinnuoye Agbaje アドウェールアキノエアグバエ あどうぇーるあきのえあぐばえ');
  $('.angelina_j').append('Angelina Jolie アンジェリーナジョリー あんじぇりーなじょりー');
  $('.anthony_h').append('Anthony Hopkins アンソニーホプキンス あんそにーほぷきんす');
  $('.anthony_m').append('Anthony Mackie アンソニーマッキー あんそにーまっきー');
  $('.benedict_c').append('Benedict Timothy Carlton Cumberbatch ベネディクト・カンバーバッチ べねでぃくとかんばーばっち');
  $('.benicio_d_t').append('Benicio Del Toro ベニチオデルトロ べにちおでるとろ');
  $('.brad_d').append('Brad Dourif ブラッドドゥーリフ ぶらっどどぅーりふ');
  $('.bradley_c').append('Bradley Charles Cooper ブラッドリークーパー ぶらっどりーくーぱー');
  $('.burn_g').append(' Burn Hugh Winchester Gorman バーンゴーマン ばーんごーまん');
  $('.catherine_o').append("Catherine Anne O'Hara キャサリンオハラ きゃさりんおはら");
  $('.chris_p').append('Chris Pratt Christopher Michael Pratt クリスプラット クリプラ くりすぷらっと くりぷら');
  $('.chris_e').append('Chris Evans クリスエヴァンス クリエヴァ くりすえゔぁんす');
  $('.chris_h').append('Chris Hemsworth クリスヘムズワース クリヘム くりすへむずわーす くりへむ');
  $('.christopher_e').append('Christopher Eccleston クリストファーエクルストン くりすとふぁーえくるすとん');
  $('.danny_e').append('Danny Elfman ダニーエルフマン だにーえるふまん');
  $('.david_b').append('David Michael Bautista Batista デビッドバウティスタ バティスタ でびっどばうてぃすた ばてぃすた');
  $('.david_p').append('David Prowse デヴィッドプラウズ でゔぃっどぷらうず');
  $('.evan_p').append('Evan Peters エヴァンピーターズ えゔぁんぴーたーず');
  $('.gwyneth_p').append('Gwyneth Kate Paltrow グウィネスパルトロー ぐうぃねすぱるとろー');
  $('.hugo_w').append('Hugo Weaving ヒューゴウィーヴィング ひゅーごうぃーゔぃんぐ');
  $('.ian_m').append('Sir Ian Murray McKellen サーイアンマッケラン さーいあんまっけらん');
  $('.idris_e').append('Idris Elba イドリスエルバ いどりすえるば');
  $('.james_e_j').append('James Earl Jones ジェームズアールジョーンズ じぇーむずあーるじょーんず');
  $('.james_s').append('James Spader ジェームズスペイダー じぇーむずすぺいだー');
  $('.jennifer_l').append('Jennifer Lawrence ジェニファーローレンス じぇにふぁーろーれんす');
  $('.jeremy_b').append('Jeremy Bulloch	ジェレミーブロック じぇれみーぶろっく');
  $('.jeremy_i').append('Jeremy Irons ジェレミーアイアンズ じぇれみーあいあんず');
  $('.jeremy_r').append('Jeremy Renner ジェレミーレナー じぇれみーれなー れれみ');
  $('.jim_c').append('Jim Cumming ジムカミングス じむかみんぐす');
  $('.john_r_d').append('John Rhys-Davies John Rhys Davies ジョン・リス＝デイヴィス ジョンリスデイヴィス じょんりすでいゔぃす');
  $('.jonny_l_m').append('Jonny Lee Miller ジョニーリーミラー じょにーりーみらー');
  $('.karen_g').append('Karen Gillan カレンギラン かれんぎらん');
  $('.ken_p').append('Ken Page ケンペイジ けんぺいじ');
  $('.kenny_b').append('Kenny Baker ケニーベイカー けにーべいかー');
  $('.lee_p').append('Lee Pace リーペイス りーぺいす');
  $('.mark_r').append('Mark Ruffalo マークラファロ まーくらふぁろ');
  $('.martin_f').append('Martin John Christopher Freeman マーティンフリーマン まーてぃんふりーまん');
  $('.michael_r').append('Michael Rooker マイケルルーカー まいけるるーかー');
  $('.natalie_p').append('Natalie Portman ナタリーポートマン なたりーぽーとまん');
  $('.ophelia_l').append('Ophelia Lovibond オフィリアラヴィボンド おふぃりあらゔぃぼんど');
  $('.orlando_b').append('Orlando Bloom オーランドブルーム おーらんどぶるーむ');
  $('.paul_r').append('Paul Rudd ポールラッド ぽーるらっど');
  $('.paul_re').append('Paul Reubens ポールルーベンス ぽーるるーべんす');
  $('.peter_s').append('Peter Serafinowicz ピーターセラフィノイス ぴーたーせらふぃのいす');
  $('.ray_p').append('Ray Park レイパーク れいぱーく');
  $('.rene_r').append('Rene Russo レネルッソ れねるっそ');
  $('.robert_d_j').append('Robert Downey Jr. RDJ ロバートダウニージュニア ろばーとだうにーじゅにあ');
  $('.rowan_a').append('Rowan Atkinson ローワンアトキンソン ろーわんあときんそん');
  $('.sam_r').append('Sam Riley サムライリー さむらいりー');
  $('.samuel_l_j').append('Samuel L. Jackson サミュエルエルジャクソン さみゅえるえるじゃくそん');
  $('.scarlett_j').append('Scarlett Johansson スカーレットヨハンソン スカヨハ すかーれっとよはんそん');
  $('.sebastian_s').append('Sebastian Stan セバスチャンスタン セバスタ せばすちゃんすたん せばすた');
  $('.simon_p').append('Simon Pegg サイモンペグ さいもんぺぐ');
  $('.tadanobu_a').append('Tadanobu Asano 浅野忠信 あさのただのぶ');
  $('.temuera_m').append('Temuera Morrison テムエラモリソン てむえらもりそん');
  $('.tobey_m').append('Tobey Maguire トビーマグワイア とびーまぐわいあ');
  $('.toby_j').append('Toby Jones トビージョーンズ とびーじょーんず');
  $('.tom_h').append('Tom Hiddleston Thomas William Hiddleston トムヒドルストン とむひどるすとん');
  $('.vin_d').append('Vin Diesel ヴィンディーゼル ゔぃんでぃーぜる');
  $('.william_d').append('William Dafoe ウィレムデフォー うぃれむでふぉー');
  $('.wyatt_o').append('Wyatt Oleff ワイアットオレフ わいあっとおれふ');
  $('.zoe_s').append('Zoe Saldana Zoë Saldaña ゾーイサルダナ ぞーいさるだな');
});



$(function(addTitle){
  $('.tab1-i').parent().prev().append('MARVEL マーベル まーべる');
  $('.tab2-i').parent().prev().append('Disney ディズニー でぃずにー');
  $('.tab2_1-i').parent().prev().append('The Lion King ライオンキング らいおんきんぐ');
  $('.tab2_2-i').parent().prev().append('The Nightmare Before Christmas ナイトメアビフォアクリスマス ないとめあびふぉあくりすます');
  $('.tab4_1-i').parent().prev().append('Pacific Rim パシフィックリム パシリム ぱしふぃっくりむ ぱしりむ');
  $('.tab4_2-i').parent().prev().append('The Lord of the Rings ロード・オブ・ザ・リング ろーどおぶざりんぐ 指輪');
  $('.tab4_3-i').parent().prev().append('The Hobbit ホビット ほびっと');
  $('.tab4_4-i').parent().prev().append('Star Trek スタートレック すたーとれっく すたとれ');















  setTimeout(addTitle, 1000);
});



